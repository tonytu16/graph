...
class Trip {
    ...
    void makeTrip(List<String> dests) {
        ...
        for (int i = 1; i < dests.size(); i += 1) {
            ...
            plan.setPaths();
            List<Integer> segment = plan.pathTo();
    ...
    int reportSegment(int seq, int from, List<Integer> segment) {
        ...
        double distance = 0.0;
        String dirName = "";
        String destName = _map.getLabel(
                segment.get(segment.size() - 1)).toString();

        for (int i = 0; i < segment.size() - 1; i++) {
            roadName = _map.getLabel(segment.get(i),
                    segment.get(i + 1)).toString();
            distance = _map.getLabel(segment.get(i),
                    segment.get(i + 1)).length();
            dirName =  _map.getLabel(segment.get(i),
                    segment.get(i + 1)).direction().fullName();

            while (i < segment.size() - 2
                    && _map.getLabel(segment.get(i + 1),
                    segment.get(i + 2)).toString().equals(roadName)
                    && _map.getLabel(segment.get(i + 1),
                    segment.get(i + 2)).direction()
                    .fullName().equals(dirName)) {
                distance += _map.getLabel(segment.get(i + 1),
            ...
            String result = "";
            if (i < segment.size() - 2) {
                result = String.format("" + seq + ". Take " + roadName
                        + " " + dirName + " for %.1f" + " miles.", distance);
            } else {
                result = String.format("" + seq + ". Take " + roadName
                        + " " + dirName + " for %.1f" + " miles to "
                        + destName + ".", distance);
            }
    ...
    private class TripPlan extends SimpleShortestPaths {
        ...
        protected double estimatedDistance(int v) {
            Location start = _map.getLabel(getSource());
            if (_map.contains(v)) {
                Location vLoc = _map.getLabel(v);
                return start.dist(start, vLoc);
            }
            return 0.0;
        }
